...
describe Profile do
  ...
  it "should require a first name" do
    blank_first_name_profile = @user.build_profile(@attr.merge(:first_name => ""))
    blank_first_name_profile.should_not be_valid    
  end

  it "should reject a short first name" do
    short_first_name_profile = @user.build_profile(@attr.merge(:first_name => "a"))
    short_first_name_profile.should_not be_valid    
  end

  it "should reject a long first name" do
    long_first_name_profile = @user.build_profile(@attr.merge(:first_name => "a" * 31))
  ...
  it "should require a last name" do
    invalid_last_name_profile = @user.build_profile(@attr.merge(:last_name => ""))
    invalid_last_name_profile.should_not be_valid    
  end

  it "should reject a short last name" do
    short_last_name_profile = @user.build_profile(@attr.merge(:last_name => "a"))
    short_last_name_profile.should_not be_valid    
  end

  it "should reject a long last name" do
    long_last_name_profile = @user.build_profile(@attr.merge(:last_name => "a" * 31))
  ...
  it 'should require a birth date' do
    blank_birth_date_profile = @user.build_profile(@attr.merge(:birth_date => ''))
  ...
  it 'should accept valid birth dates' do
    ...
    birth_dates.each do |date|
      valid_birth_date_profile = @user.build_profile(@attr.merge(:birth_date => date))
  ...
  it 'should reject invalid birth dates' do
    ...
    birth_dates.each do |date|
      invalid_birth_date_profile = @user.build_profile(@attr.merge(:birth_date => date))
      invalid_birth_date_profile.should_not be_valid
    end
  end
  
  it 'should require gender' do
    blank_gender_profile = @user.build_profile(@attr.merge(:gender => ''))
  ...
  it 'should accept valid genders' do
    ...
    genders.each do |gender|
      valid_gender_profile = @user.build_profile(@attr.merge(:gender => gender))
  ...
  it 'should reject invalid genders' do
    genders = ['abcd','G','123']
    genders.each do |gender|
      invalid_gender_profile = @user.build_profile(@attr.merge(:gender => gender))
      invalid_gender_profile.should_not be_valid
    end
  end
  
  it 'should accept blank biography' do
    blank_bio_profile = @user.build_profile(@attr.merge(:bio => ''))
    blank_bio_profile.should be_valid
  end

  it 'should reject long biographies' do
    valid_bio_profile = @user.build_profile(@attr.merge(:bio => 'A' * 501))
